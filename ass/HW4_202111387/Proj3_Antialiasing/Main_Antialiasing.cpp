// =============================================================================
//  Computer Graphics HW4 - Q3: Anti-aliasing (RayTracer_v1 + Gamma + AA)
//
//  Removes the "jaggies" at sphere boundaries with sampling-based anti-aliasing:
//  for each pixel we cast N = 64 random eye rays inside the pixel's area and
//  average the results with a box filter (each sample weighted 1/N). Edge
//  pixels mix object + background -> smooth boundaries. Averaging is in linear
//  space; gamma (1/2.2) is applied once at the end.
//      L = ka*Ia + sum_lights [ kd*I*max(0, n.l) + ks*I*max(0, r.v)^p ]
//  Single point light (-4,4,-3) + white ambient, hard shadows via shadow rays.
//
...
public:
    ...
    // Q3: ray through a continuous sub-pixel position (px in [0,nx], py in [0,ny])
    Ray getRay(float px, float py) const {
        float us  = l + (r - l) * px / nx;
        float vs  = b + (t - b) * py / ny;
        vec3  dir = us * u + vs * v - d * w;
        return Ray(eye, dir);
    }
};
...
void render()
{
    ...
    const float invGamma = 1.0f / 2.2f;                          // Q2: gamma = 2.2
    const int   N        = 64;                                   // Q3: samples per pixel

    // Box filter: average N random sub-pixel samples (each weight 1/N). Fixed
    // seed -> reproducible image. Averaging is done in LINEAR space, gamma last.
    ...
    for (int j = 0; j < Height; ++j)
    {
        for (int i = 0; i < Width; ++i)
        {
            ...
            for (int s = 0; s < N; ++s)
            {
                // cast a random eye ray inside the pixel's area [i,i+1) x [j,j+1)
                Ray ray = camera.getRay(i + jitter(rng), j + jitter(rng));
                Hit rec;
                if (scene.hit(ray, 0.0f, INF, rec))
                    sum += shade(scene, ray, rec);
                // a miss contributes background black (0)
            }

            vec3 color = sum / float(N);                         // Q3: box-filter average
            color = clamp(color, 0.0f, 1.0f);
            color = pow(color, vec3(invGamma));                  // Q2: gamma correction
...
int main(int argc, char* argv[])
{
    ...
    window = glfwCreateWindow(Width, Height, "HW4 Q3 - Antialiasing (64 spp)", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
