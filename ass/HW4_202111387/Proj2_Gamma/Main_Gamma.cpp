// =============================================================================
//  Computer Graphics HW4 - Q2: Gamma Correction
//
//  RayTracer_v1 (Q1) + gamma correction. After the linear Phong color is
//  clamped to [0,1], each channel is raised to the power 1/gamma (gamma = 2.2)
//  before display, which brightens the mid-tones to match perceptual response.
//      L = ka*Ia + sum_lights [ kd*I*max(0, n.l) + ks*I*max(0, r.v)^p ]
//      display = clamp(L, 0, 1) ^ (1 / 2.2)
...
int main(int argc, char* argv[])
{
    ...
    window = glfwCreateWindow(Width, Height, "HW4 Q2 - Gamma Correction", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
