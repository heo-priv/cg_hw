// =============================================================================
//  Computer Graphics HW4 - Q4: New Feature (sampling comparison, split screen)
//
//  Demonstrates the effect of anti-aliasing within a single image: the LEFT
//  half is rendered with NO anti-aliasing (1 ray at each pixel center, so the
//  sphere edges show jaggies), and the RIGHT half with 64-sample anti-aliasing
//  (smooth edges). A thin yellow divider marks the boundary. Both halves share
//  the same Phong shading + gamma pipeline from Q1-Q3.
...
void render()
{
    ...
    const float invGamma = 1.0f / 2.2f;                          // gamma = 2.2
    const int   N_AA     = 64;                                   // right half: samples/pixel
    ...
    for (int j = 0; j < Height; ++j)
    {
        ...
        {
            // Q4 feature: side-by-side sampling comparison.
            //   LEFT  half -> NO anti-aliasing: 1 ray at the pixel center (jaggy)
            //   RIGHT half -> 64-sample anti-aliasing (smooth)
            bool aa = (i >= half);
            int  n  = aa ? N_AA : 1;
            ...
            for (int s = 0; s < n; ++s)
            {
                Ray ray = aa ? camera.getRay(i + jitter(rng), j + jitter(rng))
                             : camera.getRay(i, j);              // center sample (aliased)
                Hit rec;
                if (scene.hit(ray, 0.0f, INF, rec))
                    sum += shade(scene, ray, rec);
            }

            vec3 color = sum / float(n);                         // box-filter average
            color = clamp(color, 0.0f, 1.0f);
            color = pow(color, vec3(invGamma));                  // gamma correction

            if (i == half)                                       // thin divider line
...
int main(int argc, char* argv[])
{
    ...
    window = glfwCreateWindow(Width, Height, "HW4 Q4 - AA Comparison (L: 1 spp | R: 64 spp)", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
