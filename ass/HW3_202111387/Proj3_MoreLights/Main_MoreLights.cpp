// =============================================================================
//  Computer Graphics HW3 - Q3: New Feature (Multiple Light Sources)
//
//  Builds on the Q2 Phong ray tracer (same scene/camera) and adds a SECOND
//  point light. Because shading already loops over scene.lights, each light
//  contributes its own diffuse + specular term and casts its own shadow ray,
//  so the scene now shows two specular highlights and two shadow directions.
//      L = ka*Ia + sum_lights [ kd*I*max(0, n.l) + ks*I*max(0, r.v)^p ]
//  Lights: (-4,4,-3) and (4,4,-3), both white, unit intensity, no falloff;
//  white unit-intensity ambient. Hard shadows via shadow rays; gamma 1/2.2.
//
...
void render()
{
    ...
    for (int j = 0; j < Height; ++j)
    {
        for (int i = 0; i < Width; ++i)
        {
            ...
            color = clamp(color, 0.0f, 1.0f);                    // clamp to displayable range
            color = pow(color, vec3(invGamma));                  // gamma correction for display
...
int main(int argc, char* argv[])
{
    ...
    window = glfwCreateWindow(Width, Height, "HW3 Q3 - Multiple Lights", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
