// =============================================================================
//  Computer Graphics HW3 - Q2: Phong Shading
//
//  Same scene/camera as Q1 (plane y=-2 + three spheres at z=-7), now shaded
//  with the Phong illumination model:
//      L = ka*Ia + sum_lights [ kd*I*max(0, n.l) + ks*I*max(0, r.v)^p ]
//  A single point light at (-4,4,-3) (white, unit intensity, no falloff) and a
//  white unit-intensity ambient light are used. Hard shadows are computed with
//  shadow rays. A final gamma correction (1/2.2) is applied for display.
//
...
int main(int argc, char* argv[])
{
    ...
    window = glfwCreateWindow(Width, Height, "HW3 Q2 - Phong Shading", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
