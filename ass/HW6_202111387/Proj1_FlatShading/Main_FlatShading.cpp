// =============================================================================
//  Computer Graphics HW6 - Q1: Flat Shading
//
//  Same software-rasterizer pipeline as HW5 (model -> camera -> perspective ->
//  viewport, triangle rasterization, depth buffer), now SHADED with the
//  Blinn-Phong model:
//      L = ka*Ia + kd*I*max(0, n.l) + ks*I*max(0, n.h)^p
//  FLAT shading: the lighting is evaluated ONCE per triangle, at its centroid,
//  using a single per-triangle (face) normal -> the whole triangle gets one
//  color (faceted look). Gamma correction (gamma = 2.2) is applied at the end.
//  OpenGL/GLFW only display the pixel buffer (glDrawPixels).
// =============================================================================
...
void render()
{
    ...
    // precompute world position and screen position of every vertex
    std::vector<vec3> world(gNumVertices), scr(gNumVertices);
    for (int i = 0; i < gNumVertices; ++i)
    {
        world[i] = vec3(Mmodel * vec4(gVertices[i], 1.0f));
    ...
    for (int tri = 0; tri < gNumTriangles; ++tri)
    {
        ...
        // ---- FLAT shading: one color for the whole triangle ----
        vec3 centroid = (world[i0] + world[i1] + world[i2]) / 3.0f;
        vec3 faceN    = cross(world[i1] - world[i0], world[i2] - world[i0]);
        if (dot(faceN, centroid - center) < 0.0f) faceN = -faceN;   // orient outward
        vec3 triColor = shade(centroid, faceN);
        triColor = pow(clamp(triColor, 0.0f, 1.0f), vec3(invGamma)); // gamma
        ...
        for (int py = miny; py <= maxy; ++py)
            for (int px = minx; px <= maxx; ++px)
            {
                ...
                if (depth > depthBuffer[idx])
                {
                    ...
                    OutputImage[idx * 3 + 0] = triColor.r;
                    OutputImage[idx * 3 + 1] = triColor.g;
                    OutputImage[idx * 3 + 2] = triColor.b;
                }
            }
    }
}
...
int main(int argc, char* argv[])
{
    ...
    window = glfwCreateWindow(Width, Height, "HW6 Q1 - Flat Shading", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
