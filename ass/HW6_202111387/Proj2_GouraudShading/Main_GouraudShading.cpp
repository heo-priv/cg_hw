// =============================================================================
//  Computer Graphics HW6 - Q2: Gouraud (per-vertex) Shading
//
//  Same rasterizer pipeline + Blinn-Phong model as Q1, but the lighting is
//  evaluated ONCE PER VERTEX (using the per-vertex normal = the unit-sphere
//  position). The three resulting vertex colors are then linearly interpolated
//  across each triangle with the barycentric weights. Gamma (2.2) per pixel.
//      L = ka*Ia + kd*I*max(0, n.l) + ks*I*max(0, n.h)^p
//  Smoother than flat, but the sharp specular highlight is softened because it
//  is only sampled at the vertices. OpenGL/GLFW only display via glDrawPixels.
// =============================================================================
...
void render()
{
    ...
    // Gouraud: shade ONCE PER VERTEX (using the per-vertex normal = the unit
    // sphere position), then interpolate the resulting colors across triangles.
    std::vector<vec3> scr(gNumVertices), vColor(gNumVertices);
    for (int i = 0; i < gNumVertices; ++i)
    {
        vec3 world  = vec3(Mmodel * vec4(gVertices[i], 1.0f));
        vec3 normal = normalize(gVertices[i]);            // unit-sphere vertex == its normal
        vColor[i]   = shade(world, normal);               // linear color at the vertex
    ...
    for (int tri = 0; tri < gNumTriangles; ++tri)
    {
        ...
        // ---- GOURAUD shading: interpolate the per-vertex colors ----
        const vec3& cA = vColor[i0]; const vec3& cB = vColor[i1]; const vec3& cC = vColor[i2];
        ...
        for (int py = miny; py <= maxy; ++py)
            for (int px = minx; px <= maxx; ++px)
            {
                ...
                if (depth > depthBuffer[idx])
                {
                    ...
                    vec3 col = a * cA + bb * cB + c * cC;            // interpolate linear color
...
int main(int argc, char* argv[])
{
    ...
    window = glfwCreateWindow(Width, Height, "HW6 Q2 - Gouraud Shading", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
